...
import { useWorkspaceDetail, useGenerateQuiz, useSubmitQuiz } from '@/hooks/useWorkspace';
...
export default function QuizArenaPage() {
  ...
  // Derive text from workspace docs for generation
  const docText = ws?.documents?.map(d => d.title).join(' ') ?? '';
  ...
  function handleStart() {
    if (!ws) return;
    // Use titles + subject as seed text; real impl uses ingested raw_text
    const seedText = ws.documents.length > 0
      ? ws.documents.map(d => d.title).join('. ') + '. ' + (ws.subject ?? '')
      : ws.subject + ' fundamentals and key concepts for exam preparation';

    generate.mutate(
      { workspace_id: id, text: seedText.repeat(5), num_questions: numQ, difficulty },
      {
        onSuccess: (res) => {
        ...
        <div className="space-y-4">
            ...
            <div className="flex gap-2 mt-1">
              {[5, 10, 15, 20].map(n => (
                <button key={n} type="button" onClick={() => setNumQ(n)}
                  className={numQ === n ? 'tag-solid' : 'tag cursor-pointer'}>{n}</button>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="flex gap-2 mt-1">
              {['easy','medium','hard','mixed'].map(d => (
                <button key={d} type="button" onClick={() => setDifficulty(d)}
                  className={difficulty === d ? 'tag-solid' : 'tag cursor-pointer capitalize'}>{d}</button>
              ))}
            </div>
          </div>
          ...
          {!ws || ws.documents.length === 0 ? (
            <p className="text-xs text-status-amber flex items-center gap-1">
              ...
              No docs ingested — quiz will generate from subject name only. Upload notes for better questions.
            </p>
          ) : (
            <p className="text-xs text-ink-faint">{ws.documents.length} source(s) loaded · {ws.subject}</p>
          )}

          <button onClick={handleStart} disabled={generate.isPending} className="btn-ink w-full justify-center">
            {generate.isPending ? <><Loader2 className="h-4 w-4 animate-spin" /> Generating quiz…</> : <><Target className="h-4 w-4" /> Start quiz</>}
          </button>
    ...
    return (
      <div className="space-y-4 animate-fade-up max-w-2xl">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-[11px] font-mono text-ink-faint shrink-0 flex items-center gap-1">
              ...
              <button
                ...
                className={`w-full text-left px-4 py-2.5 rounded-md border text-sm transition-colors ${
                  answered === opt
                    ...
                    : 'border-forge-rule bg-paper hover:bg-paper-dark text-ink'
                }`}
              >
                {opt}
              </button>
        ...
        <div className="flex gap-2">
          ...
          >
            {submit.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> :
             current === questions.length - 1 ? 'Submit quiz' : <><ChevronRight className="h-4 w-4" /> Next</>}
          </button>
          <button type="button" onClick={handleNext} className="btn-outline text-xs">
            Skip
          </button>
        </div>
      </div>
  ...
  if (stage === 'result' && result) return (
    <div className="space-y-4 animate-fade-up">
      <div className="grid gap-3 sm:grid-cols-3">
        <PaperCard className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-ink-faint">{fmtTime(elapsed)}</p>
          </div>
        </PaperCard>
        <PaperCard>
        ...
        {result.suggestions.length > 0 && (
          <ul className="mt-3 space-y-1">
            {result.suggestions.map((s, i) => (
              <li key={i} className="text-xs text-ink-faint flex items-start gap-1.5">
                <XCircle className="h-3.5 w-3.5 text-status-red mt-0.5 shrink-0" />{s}
              </li>
            ))}
          </ul>
        )}
            ...
            return (
              <div key={q.id} className="flex gap-3 text-xs">
                <div className="shrink-0 mt-0.5">
                  {correct ? <CheckCircle2 className="h-4 w-4 text-status-green" /> : <XCircle className="h-4 w-4 text-status-red" />}
                </div>
                <div>
      ...
      <button onClick={() => setStage('setup')} className="btn-outline">
